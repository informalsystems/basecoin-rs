...
gaiad --node tcp://localhost:26657 tx gov vote 1 yes --home $HOME/data/ibc-0 --keyring-backend test --keyring-dir $HOME/data/ibc-0 --chain-id ibc-0 --from validator --yes
plan_height=$(gaiad --node tcp://localhost:26657 query gov proposal 1 --home $HOME/data/ibc-0/ | grep ' height:' | awk '{print $2}' | tr -d '"')
